...
function define_property_fields_raw() {
    // This function defines the raw property fields used in the property form.
    // It returns an array of field definitions, each containing metadata about the field.
  ...
  'property_label_before' =>array(
    ...
    'property_internal_id' =>array(
                    ...
                    'type'                =>  'post_meta',
                    'meta_name'           =>  'property_internal_id',
                    'inputype'            =>  'input',
                    'bootstral_length'    =>  '6'
    ),
    'local_show_hide_price' =>array(
                    'section'             => 'description',
                    'label'               =>  esc_html__('Hide/Show Price?','wpresidence'),
                    'type'                =>  'post_meta',
                    'meta_name'           =>  'local_show_hide_price',
                    'inputype'            =>  'input',
                    'bootstral_length'    =>  '6'
    ),
          ...
          'embed_video_type' =>array(
           ...
           'embed_video_id' =>array(
            ...
            'embed_virtual_tour' =>array(
                             'section'             => 'location',
                             'label'               =>  esc_html__('Virtual Tour / Meta Reels','wpresidence'),
                             'type'                =>  'post_meta',
...
function wpestate_delete_listing_dashboard($delete_get_id,$current_user,$agent_list){
    ...
    if( !is_numeric( $delete_get_id) ){
        ...
        if( $current_user->ID != $the_post->post_author   &&  !in_array($the_post->post_author , $agent_list)  ) {
            ...
            // images deletion will be via hook in libs\property_page_functions\gallery_functions.php
...
if( !function_exists('wpestate_ajax_register_agent') ):
  ...
  function wpestate_ajax_register_agent(){
        ...
        $can_register_agent = current_user_can( 'manage_options' )
            || current_user_can( 'wpresidence_agency_role' )
            || current_user_can( 'wpresidence_developer_role' );

        if ( ! $can_register_agent ) {
            wp_send_json_error( esc_html__( "You don't have the required permissions", 'wpresidence' ) );
        }
        ...
        if($is_agent_edit!=1){
            if ( 'no' === wpresidence_get_option( 'wp_estate_allow_user_registration', 'yes' ) ) {
                wp_send_json_error( esc_html__( 'User registration is disabled.', 'wpresidence-core' ) );
            }
         ...
         $user->set_role( 'wpresidence_agent_role' );
        ...
        if (taxonomy_exists('property_category_agent')) {
         ...
         foreach ($acf_fields as $field_key => $field_value) {
            ...
            // Update the field
            $result = update_field($field_key, $field_value, $new_agent_id);
            if ($result) {
        ...
        if (taxonomy_exists('property_city_agent') && isset($agent_city) && $agent_city != 'none') {
            wp_set_object_terms($new_agent_id, $agent_city, 'property_city_agent');
        }

        if (taxonomy_exists('property_county_state_agent') && isset($agent_county) && $agent_county != 'none') {
            wp_set_object_terms($new_agent_id, $agent_county, 'property_county_state_agent');
        }

        if (taxonomy_exists('property_area_agent') && isset($agent_area) && $agent_area != 'none') {
