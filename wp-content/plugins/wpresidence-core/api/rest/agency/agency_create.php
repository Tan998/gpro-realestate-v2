<?php
/**
 * WPResidence Agency Creation API Functions
 *
 * Functions for creating new agencies via the REST API,
...
/**
 * Permission Callback for Creating a New Agency via REST API
 *
 * Validates user authentication and permission to create agencies.
 ...
 */
function wpresidence_check_permissions_for_posting_agency(WP_REST_Request $request) {
    ...
    if (!$userID || !is_user_logged_in()) {
        return new WP_Error(
            'rest_forbidden',
            esc_html__('You must be logged in to create an agency.', 'wpresidence-core'),
            ['status' => 403]
        );
    }

    // Check user capabilities for posting
    if (!current_user_can('publish_estate_agencies')) {
        return new WP_Error(
            'rest_forbidden',
            esc_html__('You do not have permission to create an agency.', 'wpresidence-core'),
            ['status' => 403]
        );
    }

    // For agencies, we could check membership or specific permissions
    // Similar to property creation but adapted for agencies
 ...
 * Create a new agency.
 * 
 * Creates an agency post with associated taxonomies, metadata, and media.
 ...
 */
function wpresidence_create_agency(WP_REST_Request $request) {
    // Retrieve mandatory fields from options
    $mandatory_fields = array('agency_name', 'agency_email');
    ...
    if (!is_email($input_data['agency_email'])) {
        return new WP_Error(
            'rest_invalid_email',
    ...
    // Get the agency name
    $agency_title = $input_data['agency_name'];
    
    // Create the agency post
    $post_id = wp_insert_post([
        'post_type'   => 'estate_agency',
        'post_title'  => $agency_title,
        'post_status' => 'publish',
        'post_author' => $current_user,
        'post_content' => isset($input_data['agency_description']) ? $input_data['agency_description'] : '',
    ]);
    ...
    foreach ($input_data as $key => $value) {
        if (taxonomy_exists($key)) {
            ...
            if ($key == 'agency_custom_data' && isset($input_data['agency_custom_data']) && is_array($input_data['agency_custom_data'])) {
                wpresidence_process_agency_custom_fields($post_id, $input_data['agency_custom_data']);
            } else if ($key=='user_registration' && is_array($input_data['user_registration'])) {
                $user_result = wpresidence_process_user_registration($post_id, $input_data['user_registration'], 'estate_agency', $input_data);
    ...
    if (!empty($input_data['featured_image'])) {
        wpresidence_set_agency_featured_image($post_id, $input_data['featured_image']);
    }
    ...
    return rest_ensure_response([
        'status'    => 'success',
        'agency_id'  => $post_id,
        'message'   => __('Agency created successfully.')
    ]);
}

/**
 * Process custom fields and save them as post meta for an agency.
 * 
 * Processes an array of custom fields with label-value pairs
 * and saves them as agency_custom_data meta entry.
 *
 * @param int $post_id The ID of the agency post.
 * @param array $custom_fields The array of custom field data containing labels and values.
 */
function wpresidence_process_agency_custom_fields($post_id, $custom_fields) {
    if (is_array($custom_fields)) {
        // Sanitize and format the custom fields
        $agency_fields_array = array();
        ...
        update_post_meta($post_id, 'agency_custom_data', $agency_fields_array);
    }
}

/**
 * Set the featured image for an agency.
 * 
 ...
 */
function wpresidence_set_agency_featured_image($post_id, $image_url) {
