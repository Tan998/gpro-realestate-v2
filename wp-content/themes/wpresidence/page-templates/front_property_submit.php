...
if( isset($_POST) && isset($_POST['action']) && $_POST['action']=='front_submit' && is_user_logged_in()) {
    ...
    if ( $paid_submission_status!='membership' || ( $paid_submission_status== 'membership' || wpestate_get_current_user_listings($parent_userID) > 0)  ){ // if user can submit
        ...
        $property_show_hide_price                   =   '';
        if(isset($_POST['property_internal_id'])){
            $property_internal_id          =   wp_kses( esc_html($_POST['property_internal_id']),$allowed_html);
        }

        $property_show_hide_price                   =   '';
        if(isset($_POST['local_show_hide_price'])){
            $property_show_hide_price          =   wp_kses( esc_html($_POST['local_show_hide_price']),$allowed_html);
        }

        if($property_show_hide_price=='yes'){
            $local_show_hide_price_check=' checked="checked" ';
        }else{
            $local_show_hide_price_check=' ';
        }
        ...
        $option_video                   =   '';
        // Allow Vimeo, YouTube, and TikTok video types
        ...
        if($post_id) {
            ...
            foreach($attchs as $att_id){
                if( !is_numeric($att_id) ){
                    ...
                    // Persist gallery order so images appear when editing the listing from the dashboard
            $clean_gallery_ids = array_map('intval', array_filter($attchs));
            update_post_meta($post_id, 'wpestate_property_gallery', $clean_gallery_ids);
            ...
            update_post_meta($post_id, 'local_show_hide_price', $property_show_hide_price);
