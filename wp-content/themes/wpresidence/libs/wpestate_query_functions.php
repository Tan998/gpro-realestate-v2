...
function wpestate_return_filtered_query($args,$do_filter){
	if($args['orderby']=='ID' || $args['orderby']=='rand'){
            $do_filter='no';
	}
...
function wpestate_create_query_meta_by_array($meta_data){
    ...
    foreach($meta_data as $meta_name=>$meta_info){
        ...
        if( isset($meta_info['value']) && !is_numeric( $meta_info['value'] ) ){
            $compare_type='';
        }

        if( $meta_name =='property_price_max') $meta_name ='property_price';

        if(isset($meta_info['key']) && isset($meta_info['value']) && isset($meta_info['compare'])){
          $temp_array=array(
            'key'     =>   $meta_info['key'],
